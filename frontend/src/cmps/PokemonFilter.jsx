...
export function PokemonFilter() {
	...
	return (
		<div className="flex flex-col h-screen bg-white p-4 dark:bg-zinc-900 text-zinc-800">
			<div className="flex justify-between items-center w-full text-zinc-700  ">
				...
				<button className=" flex items-center justify-center cursor-pointer" onClick={() => clearFilter()}>
					{svgs.clear({ className: 'w-8 h-8 dark:stroke-zinc-50' })}
				</button>
			</div>
			...
			<div className="grid grid-cols-2 gap-4 m-auto mt-32 w-full h-[60%]">
				{generations.map(generation => (
					<div
						key={generation.id}
						className={`flex flex-col items-center justify-center cursor-pointer dark:bg-zinc-800 dark:text-zinc-50 bg-zinc-50 p-2 gap-2 rounded-lg shadow-md border border-gray-200 md:flex-row ${
					...
					>
						...
						<div className="flex items-center justify-center w-40">
							<img src={generation.img} alt={generation.name} className=" max-w-30 max-h-30 object-cover " />
						</div>
					</div>
				))}
			</div>
